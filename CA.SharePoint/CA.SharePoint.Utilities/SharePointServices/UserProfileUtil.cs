...
namespace CA.SharePoint
{
    public class UserProfileUtil
    {
        ...
        //110919 added.
        //public static List<Employee> GetAllEmployeesFromSSP(string[] depts)
        //{
        ...
        //                    lstEmployees.Add(InstanceEmployee(profile, site));
        //                }
        ...
        public static List<Employee> GetEmployeeFromSSPNotInDepts(string[] depts)
        {
            var lstEmployees = new List<Employee>();
            //Employee employee = null;
            try
            {
                ...
                SPSecurity.RunWithElevatedPrivileges(delegate()
                {
                    using (var site = new SPSite(SPContext.Current.Site.ID))
                    {
                        ...
                        foreach (UserProfile profile in profileManager)
                        {
                            ...
                            //var pdepts = profile[PropertyConstants.Department].Value.ToString().ToLower().Split(new Char[] { ';', '/' });

                            //var isInDepts = false;

                            //foreach (var pdept in pdepts)
                            //{
                            //    if (depts.Contains(pdept))
                            //    {
                            //        isInDepts = true;
                            //        break;
                            //    }
                            ...
                            if (!depts.Contains(profile[PropertyConstants.Department].Value.ToString().ToLower()))
                            {
                                lstEmployees.Add(InstanceEmployee(profile, site));
                            }
                ...
                //WriteLog(string.Format("depts:{0}",
                //            string.Join(", ", depts.ToArray())),
        ...
        /// <returns></returns>
        public static Employee GetEmployeeByProp(string prop, string val)
        {
            Employee employee = null;
            try
            {
                SPSecurity.RunWithElevatedPrivileges(delegate()
                {
                    using (SPSite site = new SPSite(SPContext.Current.Site.ID))
                    {
                        ...
                        foreach (UserProfile profile in profileManager)
                        {
                            if (profile[prop].Value.AsString() == val)
                            {
                                employee = InstanceEmployee(profile, site);
                                break;
                            }
            ...
            {
                throw new Exception("根据自定义属性获取用户信息出错：" + ex.Message);
            }
        ...
        public static Employee InstanceEmployee(UserProfile profile, SPSite site)
        {
            ...
            if (string.IsNullOrEmpty(employee.PhotoUrl))
            {
                ...
                {
                    fileName =  "" + profile["EmployeeID"].Value + profile["EmployeeNameEN"].Value;
                    employee.PhotoUrl = ConfigurationManager.AppSettings["userPhotoLocation"] + fileName.Replace('.', ' ') + ConfigurationManager.AppSettings["userPhotoType"];
            ...
            employee.ManagerID = profile["ManagerId"].Value.AsString().Trim(); //20110913
            employee.JobLevel = profile["JobLevel"].Value.AsString().Trim(); //20110913
        ...
        static void WriteLog(string err, EventLogEntryType type)
        {
