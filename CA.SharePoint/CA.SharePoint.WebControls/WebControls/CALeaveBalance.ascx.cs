...
namespace CA.SharePoint.WebControls
{
    public partial class CALeaveBalance : System.Web.UI.UserControl
    {
        ...
        {
            var profileManager = new UserProfileManager(ServerContext.Current);

            //var proH = new Hashtable();

            //foreach (UserProfile profile in profileManager)
            //{
            //    if (profile[PropertyConstants.PreferredName].Value != null)
            //    {
            //        if (!proH.ContainsKey(profile[PropertyConstants.PreferredName].Value.ToString()))
            //        {
            //            proH.Add(
            //                profile[PropertyConstants.PreferredName].Value.ToString(),
            //                profile[PropertyConstants.Department].Value + ""
            //            );
            //        }
            //    }
            //}
            ...
            foreach (SPListItem item in coll)
            {
                ...
                //var theEmployee = new SPFieldLookupValue(item["Employee"] + "").LookupValue;

                var userField = item.Fields["Employee"] as SPFieldUser;
                var userFieldValue = userField.GetFieldValue(item["Employee"].ToString()) as SPFieldUserValue;

                if (userFieldValue != null && userFieldValue.User != null)
                {
                    var theEmployee = userFieldValue.User;
                    //if (proH.ContainsKey(theEmployee))
                    try
                    {
                        var theProfile = profileManager.GetUserProfile(theEmployee.LoginName);

                        if (theProfile[PropertyConstants.Department].Value != null)
                        {
                            var isShow = false;
                            ...
                            if (!string.IsNullOrEmpty(ddlDepartments.SelectedValue))
                            {
                                ...
                                {
                                    //选中的是store，不包含在departments list里的员工 做显示
                                    isShow = !alldepts.Contains(theProfile[PropertyConstants.Department].Value + "");
                                }
                                else
                                {
                                    //选中的是别的部门，匹配该部门的员工 做显示
                                    isShow = ddlDepartments.SelectedValue.ToLower() == (theProfile[PropertyConstants.Department].Value + "").ToLower();
                                }
                            }
                            else
                            {
                                ...
                                { 
                                    //如果权限里含有store，满足 匹配权限 或者 不在alldepts中的 做显示
                                    isShow = hidAssoDepts.Value.Contains(theProfile[PropertyConstants.Department].Value + "")
                                        || !alldepts.Contains(theProfile[PropertyConstants.Department].Value + "");
                                }
                                else
                                {
                                    //如果权限里不含store，满足 匹配权限 的 做显示
                                    isShow = hidAssoDepts.Value.Contains(theProfile[PropertyConstants.Department].Value + "");
                                }
                            }
    ...
    <td>{4}</td>
    <td>{5}</td>
    <td><a href='/WorkFlowCenter/Lists/LeaveBalance/EditForm.aspx?ID={6}'>edit</a></td>
</tr>",
        new string[]{
    theProfile[PropertyConstants.PreferredName].Value + "", 
    item["Year"] + "",
    item["AnnualEntitlement"] + "",
    item["SickEntitlement"] + "",
    item["AnnualBalance"] + "",
    item["SickBalance"] + "",
    item["ID"] + ""}));
                            }
        ...
        protected void Export(SPListItemCollection coll)
        {
            ...
            //foreach (UserProfile profile in profileManager)
            //{
            //    if (profile[PropertyConstants.PreferredName].Value != null && profile[PropertyConstants.Department].Value != null)
            //    {
            //        if (!proH.ContainsKey(profile[PropertyConstants.PreferredName].Value.ToString()))
            //        {
            //            proH.Add(
            //                profile[PropertyConstants.PreferredName].Value.ToString(),
            //                profile[PropertyConstants.Department].Value.ToString()
            //            );
            //        }
            //    }
            //}
            ...
            if (coll.Count > 0)
            {
                ...
                foreach (SPListItem item in coll)
                {
                    ...
                    if (userFieldValue != null && userFieldValue.User != null)
                    {
                        ...
                        try
                        {
                            ...
                            if (theProfile[PropertyConstants.Department].Value != null)
                            {
                                ...
                                if (isShow)
                                {
                                    sheet1.Cells[i, 0].Value = theProfile[PropertyConstants.PreferredName].Value + "";
                                    sheet1.Cells[i, 1].Value = theProfile[PropertyConstants.Department].Value + "";
                    ...
                    //if (proH.ContainsKey(theEmployee))
                    //{
                    //    var theDept = proH[theEmployee] + "";
                    //    if (hidAssoDepts.Value.Contains(theDept))
                    //    {
                    //        sheet1.Cells[i, 0].Value = theEmployee;
                    //        sheet1.Cells[i, 1].Value = theDept;
                    //        sheet1.Cells[i, 2].Value = item["Year"] + "";
                    //        sheet1.Cells[i, 3].Value = item["AnnualEntitlement"] + "";
                    //        sheet1.Cells[i, 4].Value = item["SickEntitlement"] + "";
                    //        sheet1.Cells[i, 5].Value = item["AnnualBalance"] + "";
