namespace CA.WorkFlow.UI.TE
{
    ...
    class TravelExpenseClaimCommon
    {
        ...
        #region NEW
        internal static void SaveDetails(DataEdit2 dataEdit, string workflowNumber)
        {
            #region
            ...
            #endregion
            string listName = "Travel Expense Claim Details";
            SPList list = WorkFlowUtil.GetWorkflowList(listName);
            JavaScriptSerializer oSerializer = new JavaScriptSerializer();
            if (dataEdit.HotelForm.Contains("},]"))
            {
                List<HotelForm> hotelFormList = oSerializer.Deserialize<List<HotelForm>>(dataEdit.HotelForm.Replace("},]", "}]"));
                BatchInsertItems_HotelForm(list, workflowNumber, hotelFormList);
            }
            if (dataEdit.MealAllowanceForm.Contains("},]"))
            {
                List<MealAllowanceForm> mealAllowanceFormList = oSerializer.Deserialize<List<MealAllowanceForm>>(dataEdit.MealAllowanceForm.Replace("},]", "}]"));
                BatchInsertItems_MealAllowanceForm(list, workflowNumber, mealAllowanceFormList);
            }
            if (dataEdit.TransportationForm.Contains("},]"))
            {
                List<Transportation_SamplePurchase_OtherForm> tsoList_Transportation = oSerializer.Deserialize<List<Transportation_SamplePurchase_OtherForm>>(dataEdit.TransportationForm.Replace("},]", "}]"));
                BatchInsertItems_Transportation_SamplePurchase_OtherForm(list, workflowNumber, tsoList_Transportation, "Local Transportation");
            }
            if (dataEdit.SamplePurchaseForm.Contains("},]"))
            {
                List<Transportation_SamplePurchase_OtherForm> tsoList_SamplePurchase = oSerializer.Deserialize<List<Transportation_SamplePurchase_OtherForm>>(dataEdit.SamplePurchaseForm.Replace("},]", "}]"));
                BatchInsertItems_Transportation_SamplePurchase_OtherForm(list, workflowNumber, tsoList_SamplePurchase, "Sample Purchase");
            }
            if (dataEdit.OtherForm.Contains("},]"))
            {
                List<Transportation_SamplePurchase_OtherForm> tsoList_Other = oSerializer.Deserialize<List<Transportation_SamplePurchase_OtherForm>>(dataEdit.OtherForm.Replace("},]", "}]"));
                BatchInsertItems_Transportation_SamplePurchase_OtherForm(list, workflowNumber, tsoList_Other, "Others");
            }
        }

        private static void BatchInsertItems_HotelForm(SPList list, string workflowNumber, List<HotelForm> hotelFormList)
        {
            ...
            foreach (HotelForm hotel in hotelFormList)
            {
                PaidByCreditCard = Int32.Parse(hotel.PaidCredit);
                SpecialApprove = 0;
                if (hotel.ExpenseType != "HK/Oversea"
                    && double.Parse(hotel.ClaimAmt) > double.Parse(hotel.ComStd))
                {
                    SpecialApprove = 1;
                }
                methodBuilder.AppendFormat(methodFormat, 0, listGuid, "New",
                                                           ...
                                                           SpecialApprove,
                                                           PaidByCreditCard,
                                                           HttpUtility.HtmlEncode(hotel.Remark),
                                                           hotel.OriginalAmt,
                                                           hotel.ClaimAmt,
                                                           hotel.ClaimAmt,
                                                           string.Empty,
            ...
            if (methodBuilder.ToString().IsNotNullOrWhitespace())
            {
                ...
                CommonUtil.logInfo(batchReturn);
            }
        }

        private static void BatchInsertItems_MealAllowanceForm(SPList list, string workflowNumber, List<MealAllowanceForm> mealAllowanceFormList)
        {
            string listGuid = list.ID.ToString();
            ...
            int PaidByCreditCard, SpecialApprove;
            foreach (MealAllowanceForm mealAllowance in mealAllowanceFormList)
            {
                PaidByCreditCard = Int32.Parse(mealAllowance.PaidCredit);
                SpecialApprove = 0;
                if (double.Parse(mealAllowance.ClaimAmt) > double.Parse(mealAllowance.ComStd))
                {
                    SpecialApprove = 1;
                }
                methodBuilder.AppendFormat(methodFormat, 0, listGuid, "New",
                                                           workflowNumber,
                                                           "Meal Allowance",
                                                           mealAllowance.ExpenseType,
                                                           string.Empty,
                                                           string.Empty,
                                                           mealAllowance.CostCenter,
                                                           mealAllowance.Currency,
                                                           mealAllowance.ExchRate,
                                                           mealAllowance.ComStd,
                                                           SpecialApprove,
                                                           PaidByCreditCard,
                                                           HttpUtility.HtmlEncode(mealAllowance.Remark),
                                                           mealAllowance.OriginalAmt,
                                                           mealAllowance.ClaimAmt,
                                                           mealAllowance.ClaimAmt,
                                                           mealAllowance.Date,
                                                           string.Empty,
                                                           mealAllowance.OtherCurrency
                   );
            }
            if (methodBuilder.ToString().IsNotNullOrWhitespace())
            {
        ...
        private static void BatchInsertItems_Transportation_SamplePurchase_OtherForm(SPList list, string workflowNumber, List<Transportation_SamplePurchase_OtherForm> tsoList, string expenseType)
        {
            string listGuid = list.ID.ToString();
            ...
            int PaidByCreditCard, SpecialApprove;
            foreach (Transportation_SamplePurchase_OtherForm tso in tsoList)
            {
                PaidByCreditCard = Int32.Parse(tso.PaidCredit);
                SpecialApprove = 0;
                methodBuilder.AppendFormat(methodFormat, 0, listGuid, "New",
                                                           workflowNumber,
                                                           expenseType,
    ...
    internal class HotelForm 
    {
        ...
        public string ComStd
        {
            set;
            get;
        }
        public string PaidCredit
        {
            set;
            get;
        }
    ...
    internal class MealAllowanceForm
    {
        public string ExpenseType
        {
            set;
    ...
    internal class Transportation_SamplePurchase_OtherForm
    {
        public string ExpenseType
        {
            set;
