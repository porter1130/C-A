...
{
    public class NewCAWorkFlowPage : Page
    {
        #region
        ...
        {
            //CommonUtil.logInfo(string.Format("[{0}]{1}", DateTime.Now.ToShortTimeString(), SPContext.Current.Web.CurrentUser.Name));
            //this.Response.Redirect("/WorkFlowCenter/Lists/Tasks/MyItems.aspx");
            SPUtility.Redirect(string.Format("/CA/MyTasks.aspx?TaskId={0}", WorkflowContext.Current.TaskID), SPRedirectFlags.Default, HttpContext.Current);

            //this.Response.Redirect(string.Format("/CA/MyTasks.aspx?TaskId={0}", WorkflowContext.Current.TaskID));
        }
                        ...
                        using (SPWeb web = site.OpenWeb("WorkFlowCenter"))
                        {
                            try
                            {
                                ...
                                if (!isValid)
                                {
                                    ...
                                    SPRoleAssignment roleAssignment = list.RoleAssignments.GetAssignmentByPrincipal(web.AllUsers[currUser]);
        ...
        private static DirectoryInfo Dir = new DirectoryInfo
                                           (Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"App_Data\ViewState\"));

        public NewCAWorkFlowPage()
        {
            if (!Dir.Exists)
                Dir.Create();
            else
