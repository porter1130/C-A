...
namespace CA.WorkFlow.UI
{
    public class CAWorkFlowPage : Page
    {
        ...
        protected void RedirectToSaveTask()
        {

            SPUtility.Redirect(string.Format("/CA/MyTasks.aspx", WorkflowContext.Current.TaskID), SPRedirectFlags.Default, HttpContext.Current);
        ...
        protected bool SecurityValidateForView()
        {
            ...
            if (applicantUser.Contains(currUser) || createdUser.Contains(currUser) || approverUsers.ToLowerInvariant().Contains(currUser.ToLowerInvariant()))
            {
                isValid = true;
            }
            else
