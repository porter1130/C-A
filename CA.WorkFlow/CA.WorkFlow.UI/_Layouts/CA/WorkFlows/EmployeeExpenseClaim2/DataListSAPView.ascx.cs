...
namespace CA.WorkFlow.UI.EmployeeExpenseClaim2
{
    public partial class DataListSAPView : BaseWorkflowUserControl
    {
        ...
        {
            ds = new DataSet();
            SPList list = SPContext.Current.Web.Lists["Employee Expense Claim SAP WorkFlow"];
            ...
            if (items.Count > 0)
            {
                ...
                DataTable wfDetailItemsDT = EmployeeExpenseClaimCommon.GetCollectionByList("EmployeeExpenseClaimSAPItemsWorkFlow").GetDataTable();
        ...
        protected void spgvWFItem_RowDataBound(Object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                e.Row.Cells[0].Text = row["ExpenseType"].AsString();
                if (OriginalExpenseType[e.Row.Cells[0].Text] != null)
                {
                    e.Row.Cells[0].Text = OriginalExpenseType[row["ExpenseType"].AsString()].AsString();
                }
            }
        }
        ...
        {
            DataTable sapDataTable = EmployeeExpenseClaimCommon.GetEmployeeExpenseClaimSAPItemsByWorkFlowNumber("Employee Expense Claim SAP WorkFlow", workFlowNumber).GetDataTable();
            DataTable sapItemDataTable = EmployeeExpenseClaimCommon.GetEmployeeExpenseClaimSAPItemsByWorkFlowNumber("EmployeeExpenseClaimSAPItemsWorkFlow", workFlowNumber).GetDataTable();
            ...
            if (sapItemDataTable != null && sapItemDataTable.Rows.Count > 0) 
            {
                ...
                foreach (DataRow dr in sapItemDataTable.Rows)
                {
                    if (dr["ExpenseType"].ToString().IndexOf("OR - employee vendor") == -1 && dr["ExpenseType"].ToString().IndexOf("OR - cash advance") == -1)
                    {
                        ...
                        expenceDetail.ItemText = employeeName + " " + OriginalExpenseType[dr["ExpenseType"].AsString()].AsString(); 
        ...
        private void WriteErrorLog(StringBuilder strLog)
        {
            ...
            try
            {
                ...
                string path = Server.MapPath("~/EEC_Log");
        ...
        private void InsertSAPNumberOrErrorMsg(string sAPNumber, string errorMsg,string workFlowNumber,string sapWorkFlowNumber)
        {
            ...
            var delegationSAPList = CA.SharePoint.SharePointUtil.GetList("Employee Expense Claim SAP WorkFlow");
            SPQuery query = new SPQuery();
            ...
            if (errorMsg != "")
            {
                emsg += eecli["RequestedBy"].ToString() + "-" + DateTime.Now.ToShortDateString() + "：" + errorMsg + " \n ";
            }
            eecli["ErrorMsg"] = emsg;
