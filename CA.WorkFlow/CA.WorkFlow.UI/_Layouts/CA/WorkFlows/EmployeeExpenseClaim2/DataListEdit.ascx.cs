    ...
    public partial class DataListEdit : BaseWorkflowUserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!this.Page.IsPostBack)
            {
                ...
                //DataBindDDL(ddlCostCenter1, CostCenters);
                //DataBindDDL(ddlCostCenter2, CostCenters);
                ...
                lblWorkFlowNumber.Text = fields["EECWWorkflowNumber"].ToString();
                ...
                //this.lblEmployeeID2.Text = employee.EmployeeID;

                //this.txtEmployeeVendor.Text = (double.Parse(fields["CashAdvanceAmount"].ToString()) - double.Parse(fields["TotalAmount"].ToString())).ToString();
                this.txtEmployeeVendor.Text = (0-double.Parse(fields["PreTotalAmount"].ToString())).ToString();
                //this.txtCashAdvance.Text = "-"+lblCashAdvanceAmount.Text;
        ...
        private DataTable GetCRTable()
        {
            ...
            CRTable.Columns.Add("EECWWorkflowNumber");
            CRTable.Columns.Add("CashAmount");
            if (txtEmployeeVendor.Text != "0")
            {
                ...
                row["EECWWorkflowNumber"] = lblWorkFlowNumber.Text.Trim();
            ...
            if (hfCashAdvanceWorkFlowNumber.Value != "")
            {
                ...
                foreach (string str in strlist) 
                {
                    if (str!="") 
                    {
                        ...
                        row1["EECWWorkflowNumber"] = lblWorkFlowNumber.Text.Trim();
        ...
        private void GetGLAccountDataTable()
        {
            ...
            //DataSet ds = EmployeeExpenseClaimCommon.GetExpenseTypeAndGLAccount(requestId);
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                {
                    strGLAccount.Append("{");
                    strGLAccount.AppendFormat("name:'{0}',val:'{1}'", dr["NewExpenseType"].ToString(), dr["GLAccount"].ToString());
                    strGLAccount.Append("},");
                }
            }
            strGLAccount.Append("]");
        ...
        private DataTable CreateItemTable()
        {
            ...
            ItemTable.Columns.Add("EECWWorkflowNumber");
            ItemTable.Columns.Add("CashAmount");
        ...
        private void UpdateItem()
        {
            ...
            foreach (RepeaterItem item in this.rptItem.Items)
            {
                ...
                //row["GLAccount"] = EmployeeExpenseClaimCommon.GetGLAccountByExpenseType(row["ExpenseType"].ToString());
        ...
        private string GetNewExpenseTypeByOriginalExpenseType(string et)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                if (dr["NewExpenseType"].ToString().IndexOf(et) != -1)
                {
            ...
            if ((item.ItemType == ListItemType.Item) || (item.ItemType == ListItemType.AlternatingItem))
            {
                ...
                if (row != null)
                {
                    ...
                    if (row["ExpenseType"].ToString() != "0")
                    {
                        ...
                        {
                            lblGLAccount.Text = EmployeeExpenseClaimCommon.GetGLAccountByExpenseType(OriginalExpenseType[row["ExpenseType"].ToString()].AsString(), GLAccount);
                        }
                    }
                    else
                    {
        ...
        private DataTable GetItemDetailsTable(DataTable itemDetails)
        {
            ...
            dt.Columns.Add("EECWWorkflowNumber");
            dt.Columns.Add("CashAmount");
            //for (int i = 0; i < itemDetails.Rows.Count; i++)
            //{
            //    if (itemDetails.Rows[i]["expenseType"].ToString() == "Employee Vendor - OP" || itemDetails.Rows[i]["expenseType"].ToString() == "Cash Advance - OR")
            //    {
            //        index.Add(i);
            //     }
            //}
            ...
            //{
            //    if (i >= itemDetails.Rows.Count)
            //    {
            //        itemDetails.Rows.RemoveAt(i - 1);
            //    }
            //    else
            //    {
            //        itemDetails.Rows.RemoveAt(i);
            //    }
            //}
            ...
            foreach (DataRow dr in itemDetails.Rows) 
            {
                if (dr["ExpenseType"].ToString().IndexOf("OR - employee vendor") == -1 && dr["ExpenseType"].ToString().IndexOf("OR - cash advance") == -1)
                {
                    ...
                    dataRow["EECWWorkflowNumber"] = dr["EECWWorkflowNumber"].ToString();
                    dataRow["CashAmount"] = dr["CashAmount"].ToString();
                }
        ...
        private void DataBindExpenseType(DropDownList ddl, DataTable dt)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ListItem li = new ListItem(dr["NewExpenseType"].ToString(), dr["NewExpenseType"].ToString());
                ddl.Items.Add(li);
            }
        }
