namespace CA.WorkFlow.UI.TravelExpenseClaim
{
    ...
    public partial class DataEdit : BaseWorkflowUserControl
    {
        ...
        {
            get { return this.ffTotalCost.Value.AsString(); }
        }
        ...
        internal string ValidateForSubmit()
        {
            ...
            if (ffCashAdvanced.Value.AsString().IsNullOrWhitespace()
                && ffComparedToApproved.Value.AsString().IsNullOrWhitespace()
                && ffNetPayable.Value.AsString().IsNullOrWhitespace()
                && ffPaidByCreditCard.Value.AsString().IsNullOrWhitespace()
                && ffTotalCost.Value.AsString().IsNullOrWhitespace()
                && ffTotalExceptFlight.Value.AsString().IsNullOrWhitespace())
            {
                errorMessage = "Claim Summary can not be null, please check it again.";
            }
            return errorMessage;
        }
    }
