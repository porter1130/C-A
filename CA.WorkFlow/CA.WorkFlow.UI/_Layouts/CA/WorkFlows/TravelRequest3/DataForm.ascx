<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="DataForm.ascx.cs" Inherits="CA.WorkFlow.UI.TravelRequest3.DataForm" %>
                            ...
                            <ItemTemplate>
                                <tr>
                                    ...
                                    <td class="cell align-center w15">
                                        <table class="inner-table">
                                            <tr>
                                                <td class="last-cell h30 p5 td_vehiclecost">
...
<script type="text/javascript">
    ...
    function PeopleEditorBind() {
        $('#<%=this.cpfUser.ClientID %>' + '_upLevelDiv').blur(function () {
            $("#<%=this.btnPeopleInfo.ClientID %>").click();
        });
    }

    function CostCenterBind() {
        $('#<%=this.UpdatePanel1.ClientID %> select.width-fix').unbind()
    ...
    function Validate() {
        ...
        $('#<%=this.UpdatePanel1.ClientID %> select.width-fix').each(function () {
            if ($(this).val() == '-1') {
                error += 'Please select the cost center.\n';
        ...
        var updatePanel1 = '#<%=this.UpdatePanel1.ClientID %>';
        var ffCashAdvanced = '#<%=this.ffCashAdvanced.ClientID%>';
        var ffPostfix = '_ctl00_ctl00_TextField';
        var totalBudget = 0;
        var flightCost = 0;
        var cashAdvancedValue = Escaped($(ffCashAdvanced + ffPostfix).val(), /,/);
        var cashAdvanced = !isNaN(cashAdvancedValue) && cashAdvancedValue.length > 0 ? parseFloat(cashAdvancedValue) : 0;
        $(updatePanel1 + ' input[class^=\'TravelInformation_Number\']').each(function () {
            totalBudget += !isNaN($(this).val()) && $(this).val().length > 0 ? parseFloat($(this).val()) : 0;
        });

        $(updatePanel1 + ' td.td_vehiclecost input').each(function () {
            var isFlight = $(this).parents('td:eq(1)').prev().find('td.ca-travel-vehicle select').val() == 'Flight' ? true : false;
            if (isFlight) {
                var flightValue = $(this).val();
                flightCost += !isNaN(flightValue) && flightValue.length > 0 ? parseFloat(flightValue) : 0;
            }
        });

        if (
        cashAdvanced.toFixed(0) > (totalBudget.toFixed(0) - flightCost.toFixed(0))) {
            error += "Cash advance can’t be greater than budget excluding cost of flight.";
            SetBorderWarn($(ffCashAdvanced + ffPostfix));
        } else {
            ClearBorderWarn($(ffCashAdvanced + ffPostfix));
        }
    ...
    $('#<%=this.cpfUser.ClientID %>' + '_browse').click(function () {
        $('#<%=this.cpfUser.ClientID %>' + '_upLevelDiv').focus();
    });
