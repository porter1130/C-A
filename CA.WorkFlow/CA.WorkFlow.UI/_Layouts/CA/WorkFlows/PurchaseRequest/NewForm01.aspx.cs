namespace CA.WorkFlow.UI.PurchaseRequest
{
    ...
    public partial class NewForm01 : CAWorkFlowPage
    {
        ...
        /// <returns></returns>
        bool CheckQuartOrder(bool isHO, string sStorePurpose)
        {
            bool isOK = true;
            if (!isHO && !PurchaseRequestCommon.IsLegalStoreRequest(sStorePurpose,DateTime.Now))//是门店并且不合法的季度订单
            {
                DataForm1.UpdateItemForQuartOrder();
                DisplayMessage("Can not request quarterly order in current month or you have requested it.");
                isOK = false;
